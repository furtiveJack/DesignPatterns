...
public class CmdLineParser {
    ...
    private static class LoggerObserver implements OptionsObserver {
    ...
    private static class MandatoryOptionObserver implements OptionsObserver {
    ...
    private static class DocumentationOptionObserver implements OptionsObserver {
    ...
    private static class ConflictsOptionObserver implements OptionsObserver {
    ...
    static class Option {
        ...
        public void setArg(String arg) throws ParseException {
            if (nbParam == 0) return;
            Objects.requireNonNull(arg);
//            if (arg.startsWith("-")) {
//                throw new ParseException("Missing parameter for option " + name, 0);
//            }
            if (currentIndex > nbParam) {
                throw new ParseException("Too much parameter for option " + name, 0);
            }
            args.add(currentIndex, arg);
            currentIndex++;
        }

        public boolean hasAllArgs() {
            return nbParam == currentIndex;
        }
        ...
        public void exec() {
            System.out.println("Option " + name + "executed with args : " + args);
            action.accept(args);
        }
    ...
    public interface ParameterRetrievalStrategy {
        ArrayList<Path> processWithStrategy(OptionsManager optionsManager, String[] arguments) throws ParseException;
    }

    public final static ParameterRetrievalStrategy STANDARD = ((optionsManager, arguments) -> {
        ArrayList<Path> files = new ArrayList<>();
        Option current = null;
        for (var arg : arguments) {
            if (current != null && arg.startsWith("-")) {
                throw new ParseException("Can't take option " + arg + " as parameter of " + current.name, 0);
            }
            var _opt = optionsManager.processOption(arg);
            if (_opt.isPresent()) {
                current = _opt.get();
                if (current.hasAllArgs()) {
                    current.exec();
                    current = null;
                }
            }
        ...
        if (current != null) {
            throw new ParseException("Missing parameter for option " + current.name, 0);
        }
        optionsManager.finishProcess();
        return files;
    });

    public final static ParameterRetrievalStrategy RELAXED = ((optionsManager, arguments) -> {
        ArrayList<Path> files = new ArrayList<>();
        Option current = null;
        for (var arg : arguments) {
            var _opt = optionsManager.processOption(arg);
    ...
    public final static ParameterRetrievalStrategy OLDSCHOOL = ((optionsManager, arguments) -> {
        ArrayList<Path> files = new ArrayList<>();
        Option current = null;
        for (var arg : arguments) {
            if (current != null) {
    ...
    public List<Path> process(String[] arguments, ParameterRetrievalStrategy strategy) throws ParseException {
        return strategy.processWithStrategy(optionsManager, arguments);
    }
}